...
def _evaluate_settings(
    ...
    if -50 < odds_max < 50 and odds_concentration < 0.1:
        return (-np.inf, -np.inf), pd.DataFrame()
    odds_american = subset["odds_decimal"].apply(decimal_to_american).dropna()
    if odds_american.empty:
        return (-np.inf, -np.inf), pd.DataFrame()
    min_bound = settings["odds_range_min_american"]
    max_bound = settings["odds_range_max_american"]
    min_binding = (odds_american <= min_bound + 10).mean()
    max_binding = (odds_american >= max_bound - 10).mean()
    if min_binding < 0.05 or max_binding < 0.05:
        return (-np.inf, -np.inf), pd.DataFrame()
...
def _optimize_settings(
    ...
    ev_candidates = sorted(set(ev_quantiles + [0.0025, 0.01, 0.02, 0.05]))
    ...
    min_ev_floor = constraint_bounds.get("ev_min_floor")
    ...
    for min_os in rating_candidates:
        ...
        for ev_min in ev_candidates:
            if max_ev is not None and ev_min > max_ev:
                continue
            if min_ev_floor is not None and ev_min < min_ev_floor:
                continue
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        ev_high = ev_min * 1.5
        if min_ev_floor is not None:
            ev_low = max(ev_low, min_ev_floor)
...
def optimize(transactions: pd.DataFrame, os_markets: pd.DataFrame, min_bets: int = 50) -> List[PortfolioResult]:
    ...
    for name, selection, min_rating, min_sample in [
        ...
        constraints = {"ev_min_floor": 0.01} if name == "Core" else {"ev_min_floor": 0.0025}
        if combos.empty:
            settings, backtest = _optimize_settings(transactions, float(min_rating), min_sample, constraints)
        else:
            ...
            settings, backtest = _optimize_settings(filtered, float(min_rating), min_sample, constraints)
    ...
    ev_floors = {"Core": 1.0, "Expansion": 0.25}
    updated_portfolios = []
    for portfolio in portfolios:
        floor = ev_floors.get(portfolio.name)
        if floor is None or portfolio.settings.get("minimum_ev", 0.0) >= floor:
            updated_portfolios.append(portfolio)
            continue
        settings = {**portfolio.settings, "minimum_ev": floor}
        source_df = transactions
        if portfolio.portfolio_markets:
            combos = pd.DataFrame(portfolio.portfolio_markets)[["league_norm", "market_norm", "sportsbook"]]
            source_df = transactions.merge(combos, on=["league_norm", "market_norm", "sportsbook"], how="inner")
        subset = _apply_settings(source_df, settings)
        if settings["betting_size_strategy"] == "Flat":
            backtest = bankroll_simulation(subset, bankroll=1000, stake_strategy="flat", max_bet_pct=settings["flat_unit_pct_bankroll"])
        else:
            backtest = bankroll_simulation(
        ...
        updated_portfolios.append(
            PortfolioResult(
                name=portfolio.name,
                settings=settings,
                portfolio_markets=portfolio.portfolio_markets,
                backtest=backtest,
            )
        )
    ...
    for portfolio in updated_portfolios:
        ...
        if portfolio.portfolio_markets:
            combos = pd.DataFrame(portfolio.portfolio_markets)[["league_norm", "market_norm", "sportsbook"]]
            source_df = transactions.merge(combos, on=["league_norm", "market_norm", "sportsbook"], how="inner")
        subset = _apply_settings(source_df, portfolio.settings)
        odds_american = subset["odds_decimal"].apply(decimal_to_american).dropna()
        if not odds_american.empty:
            min_bound = portfolio.settings["odds_range_min_american"]
            max_bound = portfolio.settings["odds_range_max_american"]
            min_binding = (odds_american <= min_bound + 10).mean()
            max_binding = (odds_american >= max_bound - 10).mean()
            if min_binding < 0.05 or max_binding < 0.05:
                lower = float(odds_american.quantile(0.1))
                upper = float(odds_american.quantile(0.9))
                if lower < upper:
                    portfolio.settings["odds_range_min_american"] = round(lower, 0)
                    portfolio.settings["odds_range_max_american"] = round(upper, 0)
        subset = _apply_settings(source_df, portfolio.settings)
        if portfolio.settings["betting_size_strategy"] == "Flat":
            backtest = bankroll_simulation(subset, bankroll=1000, stake_strategy="flat", max_bet_pct=portfolio.settings["flat_unit_pct_bankroll"])
        else:
            backtest = bankroll_simulation(
                subset,
                bankroll=1000,
                stake_strategy="os kelly",
                kelly_fraction=portfolio.settings["fractional_kelly"],
                max_bet_pct=portfolio.settings["max_bet_pct_bankroll"],
            )
        adjusted_portfolios.append(
            PortfolioResult(
                name=portfolio.name,
    ...
    if len(adjusted_portfolios) >= 2:
        core = adjusted_portfolios[0]
        expansion = adjusted_portfolios[1]
        if expansion.settings["odds_range_min_american"] > core.settings["odds_range_min_american"]:
            expansion.settings["odds_range_min_american"] = core.settings["odds_range_min_american"]
        if expansion.settings["odds_range_max_american"] < core.settings["odds_range_max_american"]:
            expansion.settings["odds_range_max_american"] = core.settings["odds_range_max_american"]
        source_df = transactions
        if expansion.portfolio_markets:
            combos = pd.DataFrame(expansion.portfolio_markets)[["league_norm", "market_norm", "sportsbook"]]
            source_df = transactions.merge(combos, on=["league_norm", "market_norm", "sportsbook"], how="inner")
        subset = _apply_settings(source_df, expansion.settings)
        if expansion.settings["betting_size_strategy"] == "Flat":
            backtest = bankroll_simulation(subset, bankroll=1000, stake_strategy="flat", max_bet_pct=expansion.settings["flat_unit_pct_bankroll"])
        else:
            backtest = bankroll_simulation(
                subset,
                bankroll=1000,
                stake_strategy="os kelly",
                kelly_fraction=expansion.settings["fractional_kelly"],
                max_bet_pct=expansion.settings["max_bet_pct_bankroll"],
            )
        adjusted_portfolios[1] = PortfolioResult(
            name=expansion.name,
            settings=expansion.settings,
            portfolio_markets=expansion.portfolio_markets,
            backtest=backtest,
        )
